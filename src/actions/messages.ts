...
export const messageActions = {
  ...
  toggleDialog: (state: boolean) =>
    createAction(MessageActionsEnum.TOGGLE_DIALOG, state),
  receiveMessages: (data: Message[]) =>
    createAction(MessageActionsEnum.RECEIVE_MESSAGES, data),
  reduceMessageId: (id: string) =>
    createAction(MessageActionsEnum.REDUCE_MESSAGE_ID, id),
  // saga triggers
  fetchMessages: () => createAction(MessageActionsEnum.FETCH_MESSAGES),
  saveMessage: (chatHistory: ChatHistory[], id?: string) =>
    createAction(MessageActionsEnum.SAVE_MESSAGE, { chatHistory, id }),
  toggleAnswer: (id: string) =>
    createAction(MessageActionsEnum.TOGGLE_ANSWER, id),
  deleteMessage: (id: string) =>
    createAction(MessageActionsEnum.DELETE_MESSAGE, id),
};
