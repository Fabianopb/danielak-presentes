...
const ProductDetail = ({ products, match, productActions, routerActions }: ProductDetailProps) => {
              ...
              <div>
                <Grid stackable={true} columns={2}>
                  <Grid.Column className={styles.frame}>
                    <ImageGallery images={products.activeProduct.image} selectedIndex={products.activeProduct.featuredImageIndex} />
                  </Grid.Column>
                  <Grid.Column className={styles.frame}>
                    <div className={`${styles.detailsContainer} flex-column cross-axis-baseline`}>
                      ...
                      <div className={styles.price}>
                        <span className={products.activeProduct.discountPrice ? styles.disabledPrice : ''}>
                          {currencyFormat(products.activeProduct.currentPrice)}
                        </span>
                        {products.activeProduct.discountPrice && currencyFormat(products.activeProduct.discountPrice)}
                      </div>
                      ...
                      {products.activeProduct.storeLink &&
                        <div className={styles.buttonContainer}>
                          <Button
                            ...
                            content="Loja Elo7"
                            labelPosition="left"
                            onClick={() => window.open(products.activeProduct ? products.activeProduct.storeLink : undefined, '_blank')}
                          />
                        </div>
                      }
                      <h3>Detalhes do produto e confecção</h3>
                      <div>Peso: {products.activeProduct.weight} g</div>
                      <div>Dimensões (cm): {products.activeProduct.width} x {products.activeProduct.depth} x {products.activeProduct.height}
                        (comprimento x largura x altura)</div>
                      <div>Quantidade mínima do pedido: {products.activeProduct.minAmount} unidades</div>
                      <div>Tempo esperado para produção: {products.activeProduct.productionTime} dias úteis</div>
                ...
                <Grid>
                  <Grid.Column className={styles.description}>
                    <div dangerouslySetInnerHTML={{__html: products.activeProduct.description}} />
                  </Grid.Column>
                </Grid>
              </div>
            ) : (
