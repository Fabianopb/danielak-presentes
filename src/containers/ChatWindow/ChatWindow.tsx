...
const initialMessage = {
  ...
  message:
    "Olá, posso ajudar? Deixe sua mensagem e seu contato que retornarei em breve!",
  step: 0,
};
...
const ChatWindow = ({ messages, messageActions }: ChatWindowProps) => {
  ...
  const answerUser = () => {
    const lastDaniMsg = findLast(
      chatHistory,
      (history) => history.speaker === "dani"
    ) as ChatHistory;
    if (lastDaniMsg.step === 0) {
      const newEntry = chatHistory.concat({
        speaker: "dani",
        message:
          "Não esqueça de deixar seu nome, telefone/whatsapp ou e-mail! :)",
        step: 1,
      });
      setChatHistory(newEntry);
      scrollToBottom();
    ...
    if (lastDaniMsg.step === 1) {
      const newEntry = chatHistory.concat([
        ...
        },
        {
          speaker: "dani",
          message:
            "Quando terminar, é só fechar esta janelinha e aguardar o retorno.",
          step: 2,
        },
      ]);
      setChatHistory(newEntry);
