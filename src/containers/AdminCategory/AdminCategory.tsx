  ...
  return (
    <div className={styles.adminCategory}>
      ...
      <Modal open={categories.isDialogOpen} onClose={categoryActions.closeDialog} size="small">
        ...
        <Modal.Content>
          <p>Tem certeza que deseja apagar a categoria <em>{categories.activeCategory && categories.activeCategory.name}</em>?</p>
        </Modal.Content>
        <Modal.Actions>
          <Button basic={true} icon={true} labelPosition="right" color="blue" onClick={categoryActions.closeDialog} >
          ...
          <Button
            ...
            labelPosition="right"
            color="red"
            onClick={() => categoryActions.deleteCategory((categories.activeCategory as Category)._id as string)}
          >
